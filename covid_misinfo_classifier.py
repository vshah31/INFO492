...
class FastMisinfoClassifier:
    ...
    def extract_features(self, text):
        ...
        misinfo_keywords = [
                         'hoax', 'conspiracy', 'truth', 'fake', 'scam', 'lie', 'microchip', '5g', 'bill gates', 
                         'coverup', 'cover-up', 'exposed', 'secret', 'hidden', 'they dont want you to know',
                           ...
                           'side effect', 'death', 'black box warning', 'informed consent', 'depopulation',
                           'tracking', 'dna altering', 'experimental', 'untested', 'rushed', 'dangerous',
                           'deadly', 'killed', 'deaths', 'adverse reaction', 'adverse event', 'injury',
                           'big pharma', 'profit', 'agenda', 'mandate', 'freedom', 'choice', 'control',
                           'censored', 'censorship', 'silenced', 'suppressed', 'banned', 'truth-teller',
                           'whistleblower', 'cover up', 'fauci', 'gates', 'who', 'cdc', 'fda', 'corrupt']
        ...
        science_keywords = ['study', 'research', 'evidence', 'scientist', 'doctor', 'expert', 'data', 'clinical',
                            ...
                            'controlled', 'trial', 'sample', 'statistical', 'significant', 'efficacy',
                            'safety', 'phase', 'approved', 'authorization', 'tested', 'results',
                            'findings', 'conclusion', 'methodology', 'participants', 'patients',
                            'cohort', 'randomized', 'double-blind', 'placebo', 'vaccine', 'immunity',
                            'antibodies', 'transmission', 'prevention', 'protection', 'effectiveness']
    ...
    def extract_advanced_features(self, text):
        ...
        uncertainty_words = ['maybe', 'perhaps', 'possibly', 'allegedly', 'reportedly',
                             'supposedly', 'rumor', 'rumour', 'unconfirmed', 'unverified']
        uncertainty_count = sum(1 for word in uncertainty_words if word in text.lower())
        
        
        credible_sources = ['study', 'research', 'scientist', 'expert', 'doctor', 'professor',
                           'university', 'journal', 'published', 'peer-reviewed']
        source_credibility = sum(1 for word in credible_sources if word in text.lower())
    ...
    def _generate_explanation(self, text, prediction, probabilities, features):
        ...
        if re.search(r'side effect|death|deaths|killed|deadly|dangerous', text.lower()) and re.search(r'vaccine|vax|shot|jab', text.lower()):
            explanations.append("Claims about severe vaccine side effects without proper context")
        
        if re.search(r'microchip|track|tracking|dna|alter|change|experimental|untested', text.lower()) and re.search(r'vaccine|vax|shot|jab', text.lower()):
            explanations.append("Contains common vaccine misinformation narratives")
        
        if re.search(r'big pharma|profit|money|agenda|control|freedom', text.lower()) and re.search(r'vaccine|vax|shot|jab', text.lower()):
            explanations.append("Suggests ulterior motives behind vaccination efforts")
        
        if re.search(r'informed consent|black box|warning|not approved|emergency|authorization', text.lower()):
            explanations.append("Contains regulatory or consent-related claims that may lack context")
...
def optimize_hyperparameters(self, X_train, y_train):
    ...
    def objective(trial):
        ...
        lr = LogisticRegression(C=1.0, max_iter=1000, class_weight='balanced', solver='liblinear')
        svc = CalibratedClassifierCV(
            LinearSVC(C=svm_c, class_weight='balanced', dual=False, max_iter=1000)
        )
        rf = RandomForestClassifier(
